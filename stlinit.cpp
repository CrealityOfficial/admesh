...
static FILE* stl_open_count_facets(stl_file *stl, const char *file) 
{
  	...
  	if (fp == nullptr) {
		...
		std::cout << " stl_open_count_facets: Couldn't open " << std::endl;
    	return nullptr;
  	}
  	// Find size of file.
  	fseek(fp, 0, SEEK_END);
	...
	unsigned char chtest[128];
  	if (! fread(chtest, sizeof(chtest), 1, fp)) {
		...
		std::cout << " stl_open_count_facets:  The input is an empty file:  " << std::endl;
    	fclose(fp);
    	return nullptr;
  	}
  	...
  	if (stl->stats.type == binary) {
    	...
    	if (((file_size - HEADER_SIZE) % SIZEOF_STL_FACET != 0) || (file_size < STL_MIN_FILE_SIZE)) {
			...
			std::cout << " stl_open_count_facets: The file  " << std::endl;
      		fclose(fp);
      		return nullptr;
    	}
    	...
    	if (! header_num_faces_read || num_facets != header_num_facets)
			...
			std::cout << " stl_open_count_facets: Warning: File size doesn't match number of facets in the header: " << std::endl;
  	}
    ...
    for (uint32_t i = first_facet; i < facets_num; ++ i) {
    	...
    	if (stl->stats.type == binary) {
			...
			if (res_normal != 3 || res_outer_loop != 0 || res_vertex1 != 3 || res_vertex2 != 3 || res_vertex3 != 3 || ! endloop_ok || ! endfacet_ok) {
				...
				std::cout << " Something is syntactically very wrong with this ASCII STL!  " << std::endl;
				return false;
			}
